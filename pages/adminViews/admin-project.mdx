...
```js copy showLineNumbers filename="PageDashboard.jsx" /AdminProject/
...
```js copy showLineNumbers /AdminProjectMono/
<AdminProjectMono enableSync onSelect(console.log))} />
```

![Admin Collection](/admin-collection.png)


<Callout type="warning" emoji="⚠️">
  <p>If enabling `enableSync`, we display a sync button to synchronize local changes with a remote Git repository. </p>
  <p>This will allow you to both pull and push all changes.</p>
</Callout>
...
```js copy showLineNumbers /AdminProjectMulti/
<AdminProjectMulti onSelect={console.log} />
```


![Admin Project](/admin-project.png)
